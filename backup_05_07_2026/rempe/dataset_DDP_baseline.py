...
def get_loaders(
    ...
    """
    # 1. Instanciar DataLoaders sem as otimizações do Teste 1
    dataloader_params = {
        "num_workers": 8, 
        "pin_memory": False, #alterado
        # REMOVIDO: prefetch_factor=2 #alterado
        # REMOVIDO: persistent_workers=True #alterado
    }

    train_dataset = SegmentationDataset(train_paths, train=True)
    val_dataset = SegmentationDataset(val_paths, train=False)

    # 2. Instanciamos os Samplers Distribuídos (Padrão DDP)
    train_sampler = DistributedSampler(train_dataset)    
    val_sampler = DistributedSampler(val_dataset, shuffle=False, drop_last=True)

    # 3. Passamos os samplers
