# -*- coding: utf-8 -*-
# código alterado para lidar com os exames de treino e validação já todos em 128x128x128
import torch
...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # 1. Carregar os ficheiros (já em 128x128x128 e RAS do disco)
        image_nifti = nib.load(self.path_list["image_path"][idx])
        mask_nifti = nib.load(self.path_list["mask_path"][idx])
        
        # 2. Extrair a forma original 
        orig_shape = image_nifti.shape
        
        # 3. Opcional: Aviso de segurança
        volume_voxels = orig_shape[0] * orig_shape[1] * orig_shape[2]
        if volume_voxels > 2500000: 
            exam_name = os.path.basename(os.path.dirname(self.path_list["image_path"][idx]))
            print(f"[DataLoader Worker] ⚠️ ATENÇÃO! Exame pesado a ser lido: {exam_name} | Dimensões: {orig_shape}")

        # 4. Construir o Subject DIRETAMENTE com o image_nifti e mask_nifti
        subject = tio.Subject(
            image=tio.ScalarImage(
                tensor=image_nifti.get_fdata()[None, ...].copy(), affine=image_nifti.affine
            ),
            mask=tio.LabelMap(
                tensor=mask_nifti.get_fdata()[None, ...].copy(), affine=mask_nifti.affine
            ),
        )
        ...
        if self.train:
            ...
            # Deixado comentado por agora para o teste de baseline limpa
...
def get_loaders(
    ...
    dataloader_params = {
        ...
        "pin_memory": True,
        "prefetch_factor": 2,
        "persistent_workers": True,  # adicionar — evita re-spawn dos workers a cada epoch
    }

    #recebe os datasets pelos paths
    train_dataset = SegmentationDataset(train_paths, train=True)
    val_dataset = SegmentationDataset(val_paths, train=False)

    #n
    # 2. Instanciamos os Samplers Distribuídos
    # O train_sampler faz shuffle por omissão. então depois ao fazer o loader não precisamos de ativar o shuffle
    train_sampler = DistributedSampler(train_dataset)    
    '''
    O Comportamento Padrão (drop_last=False -> Faz Padding)
    Por omissão, para evitar que qualquer exame fique de fora, o DistributedSampler faz Padding (Preenchimento). Ele calcula o próximo múltiplo de 8 (que é 808)
    e adiciona cópias dos 3 primeiros exames do dataset ao final da lista, perfazendo 
    808 amostras no total. Assim, o sampler distribui exatamente 101 exames para cada GPU.
    '''

    # Na validação não precisamos de aleatoriedade, logo shuffle=False, usamos drop_last=True para não inflacionar os resultados ao examinar os mesmos exames repetidamente.
    val_sampler = DistributedSampler(val_dataset, shuffle=False, drop_last=True)

    # 3. Passamos os samplers e forçamos shuffle=False explicitamente
