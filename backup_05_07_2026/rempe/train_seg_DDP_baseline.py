    ...
    def train_fn(self) -> None:
        ...
        for batch_idx, data_dict in enumerate(self.profiler.iter_dataloader(loop)):
            ...
            with self.profiler.phase("data_to_gpu"):
                data = data_dict["image"].to(device=self.device, non_blocking=False) #alterado
                targets = data_dict["mask"].to(device=data.device, non_blocking=False) #alterado
            ...
            with self.profiler.phase("backward"): 
                self.optimizer.zero_grad(set_to_none=False) #alterado
                loss.backward()
...
if __name__ == "__main__":
    torch.backends.cuda.matmul.allow_tf32 = False   # ← ADICIONAR
