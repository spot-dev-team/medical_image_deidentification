...
def preparar_pares_exames(dir_defaced, dir_original, dir_pares):
    ...
    for f in os.listdir(dir_defaced):
        ...
        if os.path.exists(path_orig_folder) and os.path.isdir(path_orig_folder):
            if os.path.exists(os.path.join(path_orig_folder, "raw.nii")):
                path_orig = os.path.join(path_orig_folder, "raw.nii")
            elif os.path.exists(os.path.join(path_orig_folder, "image.nii")):
                path_orig = os.path.join(path_orig_folder, "image.nii")
                
        # 2. TENTATIVA B: Formato Ficheiros Soltos
        if path_orig is None:
            possibilidades = [
                os.path.join(dir_original, f"{exam_id}.nii"),
                os.path.join(dir_original, f"{exam_id}_image.nii"),
                os.path.join(dir_original, f"{exam_id}_0000.nii"),
                os.path.join(dir_original, f"{exam_id}_raw.nii")
            ]
        ...
        target_defaced = os.path.join(target_folder, f"{exam_id}_defaced.nii")
        target_original = os.path.join(target_folder, f"{exam_id}_original.nii")
...
def gerar_renders_lote(dir_pares):
    ...
    for exam_id in meus_exames:
        ...
        for f in os.listdir(exam_folder):
            if not f.endswith(".nii") or "_mask" in f: continue
                
            nifti_path = os.path.join(exam_folder, f)
            output_path = os.path.join(exam_folder, f.replace(".nii", "_image.png"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir_defaced", type=str, required=True, help="Pasta contendo os ficheiros anonimizados (_anon.nii)")
