...
class PhysicalEvalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_nifti_ras = resample_to_ras(nib.load(img_path, mmap='r'))
        gt_nifti_ras = resample_to_ras(nib.load(mask_path, mmap='r'))
...
def run_physical_evaluation(weights_path, out_dir, metrics_csv, metrics_txt):
    ...
    # -----------------------------------------------------------------
    # AJUSTE I/O: Mudar extensão de .nii.gz para .nii
    # -----------------------------------------------------------------
    ...
    with torch.no_grad():
        for batch in loop:
            ...
            header_nifti = resample_to_ras(nib.load(img_path, mmap='r'))
            ...
            # EXPORTAÇÃO 3: O Exame Anonimizado 
