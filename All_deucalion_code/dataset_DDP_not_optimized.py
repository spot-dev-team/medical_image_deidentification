...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # 1. Carregar os ficheiros sem fazer resample ainda
        image_nifti = nib.load(self.path_list["image_path"][idx])
        mask_nifti = nib.load(self.path_list["mask_path"][idx])
        
        # 2. Extrair a forma original (ex: (150, 150, 150) ou (512, 512, 200))
        orig_shape = image_nifti.shape
        
        # 3. Opcional: Para não poluir o terminal, imprimimos um aviso APENAS 
        # se o exame for invulgarmente grande (ex: volume superior a 5 milhões de voxels)
        # 128x128x128 = 2,097,152 voxels. Se for muito maior, alertamos!
        volume_voxels = orig_shape[0] * orig_shape[1] * orig_shape[2]
        
        if volume_voxels > 2500000: # Ajusta este limiar se necessário
        ...
        # 4. Só agora aplicamos o resample (que é o que causa o atraso)
        image = resample(image_nifti)
        mask = resample(mask_nifti)
